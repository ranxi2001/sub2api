...
在 **账号管理 > 编辑 OpenAI OAuth 账号 > Excel / BPS 协议** 中勾选 **图片支持关闭时忽略图片输入** 并保存，也支持批量编辑。账号 API 配置为 `extra.openai_excel_bps_ignore_images: true`，默认关闭，无需数据库迁移。

只有请求实际走此账号的 BPS 协议，且 **系统设置 > 功能开关 > Excel / BPS 图片支持** 已关闭时，此选项才生效。全局图片支持重新开启后，仍按原有中转或原生附件方式处理图片；无需清除账号选项。

客户端会在后续请求中重发展开后的会话历史。一次截图进入用户消息或工具结果后，即使最新输入只有文字，历史里的 `data:image` 仍会触发 `basispoints_request_invalid`，例如 `path=input[1032].output[1]`。反复重试会带回同一张历史图片，导致整个会话无法继续。此选项在图片校验和上传之前过滤历史，使已有会话可以继续以文本交互。

- 移除消息 `content` 和 `function_call_output` / `custom_tool_call_output` 的 `output` 数组中的所有 `input_image`，包括 data URL、HTTPS URL 和附件 `file_id`。不会读取、解码、上传或中转这些图片。
- 保留相邻文本、消息顺序、工具调用及 `call_id`。仅含图片的消息或工具结果改为固定的图片已忽略提示，避免空内容被拒绝或丢失工具配对；模型无法理解被忽略图片的视觉内容。
...
- 管理员可设置请求体上限 (1–128 MiB)、估算资源预算 (512–2048 MiB) 和每进程最大在途请求数 (1–512), 默认分别为 64 MiB、1024 MiB 和 128 个. 预算必须至少为请求体上限的 8 倍; 已配置的更小网关上限仍生效. 已知长度按请求体大小的 8 倍预留, 最小按 1 MiB 计算; 未知长度的未压缩请求随读取量增加预留, 压缩请求在解压期间按配置的请求体上限预留. 正文读取完成后, 未知长度和压缩请求按实际大小保留预算至请求结束. 大请求可用并发低于配置上限.
...
- 后台在应用启动后按已启用、可调度的 BPS 代理账号并发数汇总目标；Mihomo 与 IP 管理分别维护池，每池最多 4096 个目标。配置变更由后台周期同步，不由用户请求触发。并发值非正时按 1 个目标处理。
- 用户请求只从已通过初筛且未过期的节点中按质量与负载分配；数量不足时复用就绪出口。池空快速返回 503，运维 reason 为 warm_pool_empty，candidates_checked 为 0；不临时探测冷节点、不回退直连。
- 后台每池串行补充，网络探测最多 4 路并行，每轮有 8 秒预算。失败候选轮转，避免一直探测同一批慢节点；单节点探测仍有 4 秒预算。认证信息不发送到 BPS 初筛请求，代理自身认证仅用于建立隧道。
- 初筛使用固定 BPS Responses 地址的无凭据 POST，只有 JSON 401 认证挑战通过。403、407、429、重定向、异常页面、TLS/隧道失败不通过；明确拒绝至少冷却 5 分钟。该检查不调用模型，不能证明指定账号/模型的权限或长流必然稳定。
- 就绪有效期为 2 分钟，后台在到期前刷新；目标降低后不再续期多余的空闲节点。历史业务完成率优先于协议连通率；实际断流立即移出就绪池，冷却后需后台复核，用户请求不能自行恢复它。
- 会话亲和、在途请求不换出口、国家过滤、普通节点 5–30 分钟及动态节点 30 秒–2 分钟的断流冷却保持原有规则。动态供应商仍必须提供粘性出口；本地校验窗口不是供应商的实际轮换时刻。
- 后台汇总记录 warm_pool_refresh（目标、就绪、补充、冷却及失败分类），避免按用户请求展开数十条候选探测警告。管理页面显示两种来源的 Warm IP pool 状态。
- HTTP/2 降级只缓解已确认的传输故障；它不能解除供应商或 BPS 对出口的拒绝。节点预热与实际模型业务验证分开报告。
