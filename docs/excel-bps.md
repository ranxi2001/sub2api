...
- 已验证的健康会话保持出口绑定，多个已验证出口按质量与实时负载分配；空闲但健康缓存已过期的会话可参加并行选路。只有胜出的出口建立会话占用，候选探测不占用会话容量。
- 单次探测预算耗尽或模型请求发生连接故障后，节点至少冷却 15 秒，避免被其他会话立即重复选中。连续失败和断流保留更长的原有冷却；恢复仍需两次连续成功探测。客户端取消、自身截止时间到达以及其他节点胜出导致的取消，不计作节点故障。
- 同一候选节点的失败确认只记录一条汇总警告，包含探测次数与候选预算是否耗尽。最终请求失败仍保留在运维错误记录中，不以日志过滤代替故障处理。
- Responses 选路失败的诊断增加 `acquisition_reason` 和 `candidates_checked`，区分候选探测耗尽、总预算超时、无合格节点、同会话在途请求等待释放、会话容量限制、管理器或监听器不可用。客户端仍收到通用 503，不暴露代理凭据。
- 已发送的模型请求不因上述选路修复而重放；在途会话仍保持出口绑定，不切换为直连或越过国家过滤。供应商出口持续不可用时仍会失败，部署后需另行验证真实链路。
