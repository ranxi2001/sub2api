export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSMihomoDesc: '默认关闭。IP 质量差或出口不稳定会显著增加请求失败、流中断和等待时间，请谨慎开启。使用 Mihomo 代理池前需先完成配置并启动；使用 IP 管理代理池则无需启动 Mihomo。有会话标识时绑定出口，无标识时仅为本次请求临时分配，请求结束即释放。新会话按近期成功率、连通率和负载选节点，健康会话保持原出口。普通节点首次断流冷却 5 分钟，重复断流最长冷却 30 分钟；动态代理首次 30 秒，最长 2 分钟，冷却后仍需通过探测。受影响会话会在进行中的请求结束后换出口。仅确认请求尚未发送时额外重试一次，已发送或已经开始输出的请求不重放，不回退直连。空闲 30 分钟释放绑定，重启后重新分配。动态 IP 供应商必须支持粘性出口；连通探测通过不代表长流一定稳定。关闭后恢复账号原代理。',
      ...
      pelicanTest: {
        ...
        stateProbeQuestion: '状态探针（几秒判满血/降智）',
        stateProbeHint: '不发题目：每次跑一发门票探针，结果记为满血 / 降智 / 无法判断。只适用于 OpenAI 订阅（OAuth）账号；同一账号同一时刻只跑一次，并行固定为 1。',
        stateProbeBadge: '探针',
        ...
        modeProbe: '探针测试',
        probe: {
          intro: '只发两句极短的请求，几秒出结论：先裸发打一张门票，再带这张票续接一发，上游回了新门票就判为降智。',
          scheduleHint: '想定时自动跑：切到「题目测试」→「定时测试」→「添加计划」，题目选「状态探针」。',
          unsupported: '探针只适用于 OpenAI 订阅（OAuth）账号，当前账号请用题目测试。',
          modelHint: '默认 gpt-6-astra，会按账号的模型映射转换；免费号不支持该模型时会提示「模型不支持」。',
          start: '开始探针',
          probing: '探测中...',
          running: '正在探测，通常几秒内完成，最长约 90 秒',
          empty: '点击「开始探针」，结果会显示在这里。',
          verdictHealthy: '满血',
          verdictDegraded: '降智',
          verdictInconclusive: '无法判断',
          reason: '结论说明',
          failure: '失败原因',
          detail: '上游原文',
          mintStatus: '打票（第一发）',
          continueStatus: '续接（第二发）',
          ticketLength: '门票长度',
          newTicket: '续接回新门票',
          yes: '是',
          no: '否',
          model: '探测模型',
          reportedModel: '上游回报模型',
          latency: '耗时',
          finishedAt: '完成时间',
          limitation: '判据只看门票头（x-codex-turn-state），不看回报的模型名——降到 luna 时它仍会显示 astra。单次结果受当次上游波动影响，建议结合多次结果或题目测试一起看。',
          requestFailed: '探针请求失败',
          failures: {
            unsupported: '账号不适用',
            account_error: '账号认证异常',
            rate_limited: '被上游限流',
            model_unsupported: '模型不支持',
            upstream_error: '上游返回异常',
            network_error: '网络或代理异常',
            stream_error: '回复流未正常结束',
            no_ticket: '未拿到门票',
            cancelled: '已取消'
          }
        }
