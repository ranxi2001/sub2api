export default {
    accounts: {
      ...
      pelicanTest: {
        ...
        stateProbeQuestion: 'State probe (full capability or degraded in seconds)',
        stateProbeHint: 'No question is sent: each run fires one turn-state probe and records Full capability / Degraded / Inconclusive. OpenAI subscription (OAuth) accounts only; one probe per account at a time, so parallel runs are fixed at 1.',
        stateProbeBadge: 'Probe',
        ...
        modeQuestion: 'Question test',
        modeProbe: 'Probe test',
        probe: {
          intro: 'Sends two tiny requests and answers in seconds: mint a turn-state ticket, then continue with it. A new ticket on the continuation means the account is degraded.',
          scheduleHint: 'To run it on a schedule: switch to "Question test" → "Scheduled tests" → "Add plan" and pick "State probe" as the question.',
          unsupported: 'The probe only works for OpenAI subscription (OAuth) accounts. Use the question test for this account.',
          modelHint: 'Defaults to gpt-6-astra and follows the account model mapping. Free plans that do not support the model report "Model not supported".',
          start: 'Start probe',
          probing: 'Probing...',
          running: 'Probing, usually a few seconds and at most about 90 seconds',
          empty: 'Click "Start probe" and the result will appear here.',
          verdictHealthy: 'Full capability',
          verdictDegraded: 'Degraded',
          verdictInconclusive: 'Inconclusive',
          reason: 'Explanation',
          failure: 'Failure',
          detail: 'Upstream message',
          mintStatus: 'Mint (first request)',
          continueStatus: 'Continue (second request)',
          ticketLength: 'Ticket length',
          newTicket: 'New ticket on continue',
          yes: 'Yes',
          no: 'No',
          model: 'Probe model',
          ...
          limitation: 'The verdict only uses the turn-state header (x-codex-turn-state), not the reported model name, which still says astra after a downgrade to luna. A single result can be affected by upstream noise; compare several runs or the question test.',
          requestFailed: 'Probe request failed',
          failures: {
            unsupported: 'Account not supported',
            account_error: 'Account authentication error',
            rate_limited: 'Rate limited upstream',
            model_unsupported: 'Model not supported',
            upstream_error: 'Upstream error',
            network_error: 'Network or proxy error',
            stream_error: 'Response stream did not finish',
            no_ticket: 'No ticket minted',
            cancelled: 'Cancelled'
          }
        }
      },
